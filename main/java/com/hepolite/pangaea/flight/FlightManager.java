...
public class FlightManager extends Listener
{
	...
	private final void handleFlightPermission(Player player)
	{
		...
		{
			PlayerAllowFlightEvent event = new PlayerAllowFlightEvent(player);
			post(event);
			warnPlayerOfExhaustion(player);
			if (!event.canFly() || checkFlightInWater(player) || checkFlightVeryExhausted(player) || checkFlightMildlyExhausted(player) || checkFlightPotionEffects(player) || checkFlightSpaceRequirements(player))
			{
	...
	private final boolean checkFlightInWater(Player player)
	{
		...
		if (data.has("FlightManager.invertAir"))
